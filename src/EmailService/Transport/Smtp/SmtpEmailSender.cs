...
namespace EmailService.Transport.Smtp;
...
public class SmtpEmailSender(IOptions<SmtpOptions> options, ILogger<SmtpEmailSender> logger) : IEmailSender
{
    ...
    public async Task<SendResult> SendAsync(EmailMessage message, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            logger.LogInformation("Sent email {EmailId} to {RecipientCount} recipients", message.Id, message.To.Count);
        ...
        {
            logger.LogWarning(ex, "Permanent SMTP failure for {EmailId}", message.Id);
            return SendResult.Permanent($"{ex.ErrorCode}/{ex.StatusCode}: {ex.Message}");
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "Transient SMTP failure for {EmailId}", message.Id);
